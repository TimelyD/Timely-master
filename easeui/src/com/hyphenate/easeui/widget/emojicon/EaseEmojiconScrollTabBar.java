...
public class EaseEmojiconScrollTabBar extends RelativeLayout{
    ...
    private void init(Context context, AttributeSet attrs){
        ...
        mSendBtn.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                RxBus.get().send(BusCode.CHAT_ACTION_SEND);
            }
        });
    }

    @Subscribe(code = BusCode.CHAT_ACTION_SEND_ENABLE, threadMode = ThreadMode.MAIN)
    ...
    public void selectedTo(int position){
        ...
        for (int i = 0; i < tabList.size(); i++) {
            if (position == i) {
                tabList.get(i).setBackgroundColor(getResources().getColor(R.color.bg_t));
            } else {
                tabList.get(i).setBackgroundColor(getResources().getColor(R.color.white));
            }
        }
    }
