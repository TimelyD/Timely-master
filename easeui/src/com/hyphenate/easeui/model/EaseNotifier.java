...
//        vibrateAndPlayTone(message);
    ...
    protected void sendNotification(EMMessage message, boolean isForeground, boolean numIncrease) {
        ...
        try {
            ...
            if (notificationInfoProvider != null) {
                Intent launchIntent = notificationInfoProvider.getLaunchIntent(message);
                //返回null，说明是信息加锁了的,或者有多人发过来
                if(launchIntent != null && fromUsers.size()<1)
                   msgIntent = launchIntent;
            }
...
//            mBuilder.setFullScreenIntent(pendingIntent, false);
            int notifyFlag = Notification.DEFAULT_ALL;
            EaseUI.EaseSettingsProvider settingsProvider = EaseUI.getInstance().getSettingsProvider();
            boolean msgSoundAllowed = settingsProvider.isMsgSoundAllowed(null);
            boolean msgVibrateAllowed = settingsProvider.isMsgVibrateAllowed(null);
            if(msgSoundAllowed && msgVibrateAllowed){

            }else if(msgSoundAllowed){
                notifyFlag = Notification.DEFAULT_SOUND;
            }else if(msgVibrateAllowed){
                notifyFlag = Notification.DEFAULT_VIBRATE;
            }else {
                notifyFlag = Notification.DEFAULT_LIGHTS;
            }
            //以下两行使通知弹出
            mBuilder.setDefaults(notifyFlag);
