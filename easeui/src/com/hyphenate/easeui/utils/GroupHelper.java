...
public class GroupHelper {
     ...
     * @param
     */
    public static void sendKickedMsg(EMGroup group,String content/*,String[] members*/){
        String myUid = EMClient.getInstance().getCurrentUser();
        EMMessage message = EMMessage.createSendMessage(EMMessage.Type.TXT);
        message.setAttribute(EaseConstant.MESSAGE_ATTR_IS_KICKED_GROUP,myUid);
        message.setFrom(EMClient.getInstance().getCurrentUser());
        message.setTo(group.getGroupId());
     ...
     /*   StringBuilder sb = new StringBuilder();
        ...
        message.setChatType(EMMessage.ChatType.GroupChat);
        message.setUnread(true);Log.i("踢出",sb.toString()+"");
        EMClient.getInstance().chatManager().sendMessage(message);*/
    }
    ...
    public static String parseKickedMsg(EMMessage message) {
        ...
        String myUid = EMClient.getInstance().getCurrentUser();
        String id = message.getStringAttribute(EaseConstant.MESSAGE_ATTR_IS_KICKED_GROUP, null);
        String ownerNickname = message.getStringAttribute("group_ownerNickname", null);
        Log.i("dcz",msg+"+"+ownerNickname);
        if(myUid.equals(id)){
            return "你将"+msg+"移除群聊";
        }else {
            return ownerNickname+"将"+msg+"移除群聊";
        }
    }
}
