...
public class DemoDBManager {
    ...
    synchronized public void saveContact(EaseUser user){
        ...
        if(user.getIsLock()!=-1) {
            ...
            values.put(UserDao.COLUMN_NAME_CHATID, user.getChatid());
            values.put(UserDao.COLUMN_NAME_SEX, user.getChatidsex());
            values.put(UserDao.COLUMN_NAME_STATE, user.getChatidstate());
        }
        if(db.isOpen()){
            db.replace(UserDao.TABLE_NAME, null, values);
        }
...
//     * @param user
//     */
//    synchronized public void getContact(String username){
//        SQLiteDatabase db = dbHelper.getWritableDatabase();
////        db.execSQL("SELECT * FROM "+UserDao.TABLE_NAME+" WHERE "+ UserDao.COLUMN_NAME_ID +" = "+username);
//        Cursor cursor = db.query(UserDao.TABLE_NAME, new String[]{UserDao.COLUMN_NAME_NICK, UserDao
//                .COLUMN_NAME_AVATAR}, UserDao.COLUMN_NAME_ID + "=?", new String[]{username}, null, null, null);
//        ContentValues values = new ContentValues();
//        values.put(UserDao.COLUMN_NAME_ID, user.getUsername());
//        if(user.getNick() != null)
//            values.put(UserDao.COLUMN_NAME_NICK, user.getNick());
...
//    }
    ...
    public synchronized Integer saveMessage(InviteMessage message){
        ...
        if(db.isOpen()){
            ...
            values.put(InviteMessgeDao.COLUMN_NAME_MSG_ID, message.getMessageId());
    ...
    synchronized public List<InviteMessage> getMessagesList(){
        ...
        if(db.isOpen()){
            ...
            while(cursor.moveToNext()){
                ...
                String message_id = cursor.getString(cursor.getColumnIndex(InviteMessgeDao.COLUMN_NAME_MSG_ID));
    ...
    synchronized public void deleteMessage(String from, boolean whenStatusBeInvited){
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        if(db.isOpen()){
            db.delete(InviteMessgeDao.TABLE_NAME, InviteMessgeDao.COLUMN_NAME_FROM + " = ? and "+InviteMessgeDao.COLUMN_NAME_STATUS+ " = ?", new String[]{from, ""+InviteMessage.InviteMesageStatus.BEINVITEED.ordinal()});
        }
    }
