...
public class MultiImageView extends LinearLayout {
	...
	private ImageView createImageView(int position, final boolean isMultiImage) {
		...
		String pic = TextUtils.isEmpty(photoInfo.thumbnail) ? photoInfo.picture : photoInfo.thumbnail;
		imageView.setId(photoInfo.picture.hashCode());
		imageView.setOnClickListener(new ImageOnClickListener(position));
		imageView.setBackgroundColor(getResources().getColor(R.color.im_font_color_text_hint));
		GlideApp.with(getContext()).load(/*photoInfo.picture*/pic)/*.thumbnail(GlideApp.with(getContext()).load(photoInfo.thumbnail).centerCrop())*/.centerCrop().diskCacheStrategy(DiskCacheStrategy.ALL).into(imageView);
		File cacheImage = GlideUtils.getCacheImage(getContext(), photoInfo.picture);
		if(cacheImage!=null)
			GlideApp.with(getContext()).load(cacheImage).placeholder(imageView.getDrawable()).centerCrop().into(imageView);
		mViews.add(position, imageView);
		return imageView;
	}
