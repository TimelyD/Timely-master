...
//		Parse.initialize(context, ParseAppID, ParseClientKey);
	...
	public void getContactInfos(List<String> usernames, final EMValueCallBack<List<EaseUser>> callback) {
		...
		if(System.currentTimeMillis() - lastTime <1000){
			callback.onSuccess(new ArrayList<EaseUser>(DemoHelper.getInstance().getContactList().values()));
			return;
		}
		...
		ApiManger2.getApiService()
				.showFriendsInfo()
				.retry(3)
				.compose(RxUtils.<HttpResult<List<UserFriendModel>>>applySchedulers())
				...
				.subscribe(new BaseObserver2<List<UserFriendModel>>() {
					@Override
					protected void onSuccess(List<UserFriendModel> userFriendModels) {
						List<EaseUser> mList = new ArrayList<EaseUser>();
                        if(userFriendModels!=null) {
                            for (UserFriendModel model : userFriendModels) {
								EaseUser user = CodeUtils.wrapUser(model);
								EaseCommonUtils.setUserInitialLetter(user);
                                mList.add(user);
                            }
                        }
